...
void A_output(struct msg message) {
    ...
    if (A.next_seq - A.base < N) {
        if (wait_queue.empty()) {
            cout << "Send: " << A.next_packet.payload << ", seq=" << A.next_packet.seqnum << " ack="
                 << A.next_packet.acknum << endl;
            ...
            while (!wait_queue.empty() && wait_queue.front().seqnum - A.base < N) {
                ...
                pkt_t.start_time = start_time;
                A_windows.push_back(pkt_t);
                cout << "Send: " << p.payload << ", seq=" << p.seqnum << " ack=" << p.acknum << endl;
                tolayer3(0, p);
            }
            if (A.next_seq - A.base < N) {
                struct pkt_time pkt_t;
    ...
    if (packet.checksum == get_checksum(&packet)) {
        for (vector<pkt_time>::iterator iter = A_windows.begin(); iter != A_windows.end(); ++iter) {
            if (iter->packet.seqnum == packet.seqnum) {
                cout << "remove pkt (seq=" << packet.seqnum << ")from buffer, expected seq="<< A.base << endl;
        ...
        if (packet.seqnum == A.base) {
            cout << "received expect pkt, shuffle window" << endl;
            stoptimer(0);
...
void A_timerinterrupt() {
    cout << "Time out, resend: " << A_windows[0].packet.payload << " ,seq=" << A_windows[0].packet.seqnum << " ,ack="
         << A_windows[0].packet.acknum << endl;
    float cur_time = get_sim_time();
...
void B_input(struct pkt packet) {
    if (packet.checksum == get_checksum(&packet)) {
        ...
        cout << "Response to sender: " << "ack=" << ack.seqnum << endl;
        tolayer3(1, ack);
        if (packet.seqnum == baseB) {
            cout << "Received expected pkt, deliver all in-order buffer" << endl;
            packet.acknum = 1;
            B_windows[0] = packet;
            ...
            cout << " next expected seq=" << baseB << endl;
        } else if (packet.seqnum > baseB && packet.seqnum < baseB + N) {
            cout << "Unexpected pkt, buffer" << endl;
            packet.acknum = 1;
